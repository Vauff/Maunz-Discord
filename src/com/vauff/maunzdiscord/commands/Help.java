...
public class Help extends AbstractCommand<MessageCreateEvent>
{
	...
	public void exe(MessageCreateEvent event, MessageChannel channel, User author) throws Exception
	{
		...
		if (args.length == 1 || (args.length == 2 && NumberUtils.isCreatable(args[1])))
		{
			...
			Message m = Util.buildPage(helpEntries, "Command List", 10, page, 0, false, false, channel, author);
	...
	public void onReactionAdd(ReactionAddEvent event, Message message) throws Exception
	{
		...
		Message m = Util.buildPage(helpEntries, "Command List", 10, listPages.get(event.getUser().block().getId()) + pageChange, 0, false, false, event.getChannel().block(), event.getUser().block());
